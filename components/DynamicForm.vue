<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    <div v-for="field in fields" :key="field.name" class="flex flex-col">
      ...
      <input v-if="field.type !== 'file'" :type="field.type" :name="field.name" v-model="formData[field.name]"
        :placeholder="field.placeholder" :pattern="field.type === 'tel' ? '(\\+84|0)[0-9]{9}' : undefined"
      ...
      <input v-else :type="field.type" :name="field.name" @change="handleFileChange"
        class="px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
    </div>
    <button :class="buttonClasses" type="submit" :disabled="loading">
      <svg v-if="loading" class="animate-spin h-5 w-5 text-white mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none"
        viewBox="0 0 24 24">
...
const handleFileChange = (event) => {
  ...
  if (file) {
    formData.value[event.target.name] = file
  }
}
